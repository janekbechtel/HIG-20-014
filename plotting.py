...
def DrawCMSLogo(pad, cmsText, extraText, iPosX, relPosX, relPosY, relExtraDY, extraText2='', cmsTextSize=0.8):
    ...
    if not outOfFrame: 
        ...
        if iPosX == 0:
            ...
            posX_ = l + 0.1
	    posY_ = 1 - t + lumiTextOffset * t + 0.032
...
def DrawLimitBand(pad, graph_dict, draw=['exp2', 'exp1', 'exp0', 'obs'], draw_legend=None, 
    ...
    legend_dict = {
        'obs' : { 'Label' : 'Observed', 'LegendStyle' : 'PL', 'DrawStyle' : 'PLSAME'},
    ...
    for key in draw:
        if False: #key in ["exp0","obs"]:
            xtemp, ytemp = R.Double(0.0), R.Double(0.0)
            graph_dict[key].GetPoint(graph_dict[key].GetN()-1,xtemp,ytemp)
            xbatches = [0,85,110,170,350,550,750,9999] if xtemp<1000. else [0,120,350,800,1300,2000,9999]
            offsets_low = [0.,2.,3.,5.,7.,10.,10.,9999] if xtemp<1000. else [0.,5.,20.,25.,40.,60.,9999]
            offsets_high = [2.,2.,5.,8.,15.,15.,5.,9999] if xtemp<1000. else [5.,15.,20.,35.,40.,60.,9999]
            x_max_cat_before = 0.0

            for i_graph,xbatch in enumerate(xbatches[:-1]):
                offset_low = offsets_low[i_graph] #(xbatches[i_graph+1]-xbatch)/8.
                offset_high = offsets_high[i_graph] #(xbatches[i_graph+1]-xbatch)/8.
                if xbatch>xtemp:
                    continue
                xvals, yvals = [], [] 
                yerr_up, yerr_down = [], []
                ...
                for bin_ in range(graph_dict[key].GetN()):
                    x_max, y_max = R.Double(0.0), R.Double(0.0)
                    graph_dict[key].GetPoint(bin_,x_max,y_max)

                    if (float(x_max))>=xbatches[i_graph+1]:
                        break
                    x_max_cat = float(x_max)
                offset_high = (xbatches[i_graph+1]-x_max_cat)*0.4
                offset_low = (xbatch-x_max_cat_before)*0.4
                if offset_high>(2*offset_low) and x_max_cat>100.:
                    offset_high = offset_low
                x_max_cat_before = x_max_cat

                for bin_ in range(graph_dict[key].GetN()):
                    x, y = R.Double(0.0), R.Double(0.0)

                    graph_dict[key].GetPoint(bin_,x,y)
                    if key is not "obs":
                        if abs(float(x)-xbatch)<0.1:

                            xvals.append(xbatch-offset_low)
                            yvals.append(float(y))
                            if "1" in key or "2" in key:
                                yerr_up.append(graph_dict[key].GetErrorYhigh(bin_))
                                yerr_down.append(graph_dict[key].GetErrorYlow(bin_))
                                xerr_up.append(graph_dict[key].GetErrorXhigh(bin_))
                                xerr_down.append(graph_dict[key].GetErrorXlow(bin_))

                    if not ((float(x))<xbatch or (float(x))>=xbatches[i_graph+1]):
                        ...
                        if "1" in key or "2" in key:
                            ...
                            xerr_down.append(graph_dict[key].GetErrorXlow(bin_))
                    if key is not "obs":
                        if abs(float(x)-float(x_max_cat))<0.1:
                            xvals.append(x_max_cat+offset_high)
                            yvals.append(float(y))
                            if "1" in key or "2" in key:
                                yerr_up.append(graph_dict[key].GetErrorYhigh(bin_))
                                yerr_down.append(graph_dict[key].GetErrorYlow(bin_))
                ...
                if "1" in key or "2" in key:
                    graph_dict[key+str(i_graph)] = R.TGraphAsymmErrors(len(xvals), array('d', xvals), array('d', yvals), array('d', xerr_down),  array('d', xerr_up),  array('d', yerr_down),  array('d', yerr_up))
                else:
                    graph_dict[key+str(i_graph)] = R.TGraph(len(xvals), array('d', xvals), array('d', yvals))
                if key == "exp0":
                    graph_dict[key+str(i_graph)].SetLineColor(12)
                    graph_dict[key+str(i_graph)].SetLineStyle(7)
                    graph_dict[key+str(i_graph)].SetLineWidth(3)
                elif key == "exp1":
                    graph_dict[key+str(i_graph)].SetFillColor(R.mygreen2)
                elif key == "exp2":
                    graph_dict[key+str(i_graph)].SetFillColor(R.myyellow2)    

                graph_dict[key+str(i_graph)].Draw(legend_dict[key]['DrawStyle'])
        else:
